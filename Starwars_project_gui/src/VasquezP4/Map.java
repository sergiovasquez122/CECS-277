...
public class Map {
    ...
    /**
     * Return the only instance of map
     * @return the only instance of map
     */
    ...
    /**
     * Draws the rectangle as a 5 x 5 grid
     * @param g graphic object
     * @param p the point of the hero
     */
    public void draw(Graphics g, Point p){
        // All rectangles are of equal size
        final int RECTANGLE_HEIGHT = 50;
        final int RECTANGLE_WIDTH = 50;
        // the center of the rectangle for giving displaying information of map
        int center = RECTANGLE_HEIGHT / 2;
        for(int i = 0;i < map.length;++i){
            for(int j = 0;j < map[i].length;++j){
                g.setColor(Color.WHITE);
                g.fillRect(RECTANGLE_HEIGHT * j , RECTANGLE_WIDTH * i,RECTANGLE_WIDTH,RECTANGLE_HEIGHT);
                g.setColor(Color.BLACK);
                g.drawRect(RECTANGLE_HEIGHT*j,RECTANGLE_WIDTH*i, RECTANGLE_WIDTH,RECTANGLE_HEIGHT);
                // hero
                if( i == p.x && j == p.y){
                    g.drawString("H", RECTANGLE_HEIGHT * j + center, RECTANGLE_WIDTH *i + center);
                }
                else if(revealed[i][j]){
                    String text = String.valueOf(map[i][j]);
                    g.drawString(text,RECTANGLE_HEIGHT*j + center,RECTANGLE_WIDTH*i + center);
                } else {
                    g.drawString(" ", RECTANGLE_HEIGHT*j + center, RECTANGLE_WIDTH * i + center);
                }
            }
        }
    }
